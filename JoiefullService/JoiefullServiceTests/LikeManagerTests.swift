...
final class LikeManagerTests: XCTestCase {
    ...
    func testisLikedReturnsTrueIfProductIsLiked() {
        let fakeProduct = FakeResponseData.fakeProduct
        
        let isLiked1 = likeManager.isLiked(for: fakeProduct)
        likeManager.toggleLike(for: fakeProduct)
        let isLiked2 = likeManager.isLiked(for: fakeProduct)
        
        XCTAssertFalse(isLiked1, "Product should not be liked initially")
        XCTAssertTrue(isLiked2, "Product should be liked after toggle")
    }
